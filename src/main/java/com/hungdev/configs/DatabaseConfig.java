 ...
 * HikariConfig(); config.setDriverClassName(env.getProperty(
 * "spring.datasource.driver-class-name"));
 * config.setJdbcUrl(env.getProperty("spring.datasource.url"));
 * config.setUsername(env.getProperty("spring.datasource.username"));
 * config.setPassword(env.getProperty("spring.datasource.password"));
 * 
 * config.setMaximumPoolSize(10); config.setMinimumIdle(2);
 * config.setIdleTimeout(30000); config.setMaxLifetime(1800000);
 * 
 ...
 * 
 * public void closeConnection(Connection conn) { if (conn != null) { try {
